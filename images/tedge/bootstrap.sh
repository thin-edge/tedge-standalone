...
#
SERVICE_RUNTIME=${SERVICE_RUNTIME:-none}

if command -V runsvdir >/dev/null 2>&1 && [ "$SERVICE_RUNTIME" != "none" ]; then
    #
...
--------------------------- Summary ---------------------------
thin-edge.io
  device.id:      ${DEVICE_ID:-not configured}
  c8y.url:        ${C8Y_URL:-not configured}
  Cumulocity IoT: https://${C8Y_URL}/apps/devicemanagement/index.html#/assetsearch?filter=*${DEVICE_ID}*"
---------------------------------------------------------------
EOT
fi
